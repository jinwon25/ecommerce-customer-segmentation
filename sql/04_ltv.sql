...
--
-- 정의 메모:
--   1) IPT는 일자 단위로 계산. 본 데이터에서 같은 고객·같은 날 다중 거래ID
--      케이스가 93.1%로 한 결제가 여러 거래ID로 분할 기록되는 적재 구조다.
--      거래ID 단위 IPT는 분할 거래를 0일로 잡아 분위가 모두 0이 되므로,
--      `(고객ID, 거래날짜)` DISTINCT로 일자 단위 압축 후 LAG로 산출한다.
--      근거는 reports/ltv_methodology.md.
--   2) ltv_metrics / ipt_metrics는 세그먼트별 + 'ALL' UNION ALL로 명시 분리.
--      GROUPING SETS도 같은 결과를 내지만 JOIN 추적이 단순해진다.
--
-- 단순 LTV 정의:
