...
- **BigQuery 한글 컬럼 처리 — 한글 식별자가 등장하는 모든 지점에 백틱 필수.** 빠진 백틱 하나만 있어도 `Illegal input character "\xxx"` 에러가 난다.
  - 컬럼 참조: `` `고객ID` ``
  - 테이블 별칭 + 컬럼: `` s.`고객ID` ``
  - 별칭 생성(`AS`): `` 1 AS `처치` ``, `` SUM(x) AS `거래금액` ``
  - `GROUP BY` · `ORDER BY` · `WHERE` 등 모든 절: `` GROUP BY `고객ID` ``, `` ORDER BY `처치` DESC ``
  - 영문·숫자·언더스코어로만 된 식별자(`GST`, `R_Score`, `Recency`)는 백틱 불필요
